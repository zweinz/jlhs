...
export const MATCHING_SUBJECTS: RulebookSubject[] = [
  ...
  { id: 'transit-route', label: 'Transit line', status: 'in-play', support: 'approximate', notes: ['SF modification: ask whether the seeker’s current transit line stops within ¼ mile of the hider’s chosen hiding station.', 'Limited/Rapid service counts only locations where that service actually stops. Route geometry is used as a conservative stop-proximity approximation; players should confirm ambiguous stops.'] },
  { id: 'station-name-length', label: 'Station’s name length', status: 'in-play', support: 'exact', notes: ['Compare the character count of each player’s nearest valid station.', 'Exclude the word “Station” and periods. Include spaces, hyphens, ampersands, slashes, and other characters.'] },
