...
The 193 spreadsheet-defined stations have touch-friendly markers and configurable hiding-zone radii (default 0.25 miles). One station, all stations, or an entire route can be marked in/out, and every question can be enabled or disabled in one action. A preset removes stations without light rail or Rapid Muni service for a core-transit-only game. Station availability updates automatically: a station turns off as soon as its hiding-radius zone has no overlap with the current feasible area. Transit lines identify themselves when tapped. The internal station regions remain available for matching calculations but are never rendered as a partition layer. Area shading is configurable between green allowed/transparent excluded and transparent allowed/red excluded; the choice is preserved in share URLs.
